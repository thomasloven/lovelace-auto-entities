  ...
  _getEntities() {
    ...
    if (this.template) {
      if (typeof this.template === "string")
        entities = entities.concat(
          this.template.split(/[\s,]+/).map(format_entities)
        );
      else entities = entities.concat(this.template.map(format_entities));
    }
    ...
    if (this._config.filter.include) {
      ...
      for (const f of this._config.filter.include) {
        ...
        let add = all_entities.filter(entity_filter(this.hass, f)).map((e) =>
          JSON.parse(
            JSON.stringify(
              new Object({
                ...e,
